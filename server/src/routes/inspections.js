...
import { mockGetInspections, mockGetInspection, mockCreateInspection, mockUpdateInspection, mockGetInspectionRecords, mockCreateInspectionRecord, mockGetDefects, mockCreateDefect, mockDeleteDefect, mockGetDashboardStats, UNIVERSAL_CHECKLIST_ITEMS } from '../storage/mockData.js';
const router = Router();
...
// 接收Base64照片并保存到服务器
router.post('/import-photo', async (req, res) => {
    try {
...
router.get('/', async (req, res) => {
    try {
        ...
        const supabaseIds = new Set(dataWithPhotos.map((item) => item.id));
        const mergedData = [
            ...dataWithPhotos,
            ...mockUniversalData.filter((item) => !supabaseIds.has(item.id))
        ];
...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        if (passRateResult.data && passRateResult.data.length > 0) {
            const passedCount = passRateResult.data.filter((i) => i.failed_items === 0).length;
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (isEmbeddedTemplate || !records || records.length === 0) {
            ...
            return res.json({
                ...
                data: {
                    ...
                    categories: [...new Set(resolvedItems.map((item) => item.category))],
                    defects: [],
                    photos: [],
                    checkedCount: 0,
                    defectCount: 0
        ...
        const { data: photos } = await client
            ...
            .eq('inspection_id', id);
        // 组合数据
        const checklist_items = (records || []).map((record) => {
            const item = record.checklist_items;
            // 从 inspection_photos 表获取该记录的照片
            const recordPhotos = photos?.filter((p) => p.record_id === record.id).map((p) => p.photo_url) || [];
            ...
            const isValidImage = (p) => {
                const lower = p.toLowerCase();
                return validImageExtensions.some(ext => lower.endsWith(ext));
            };
            const photosFromRecord = (record.photos || []).filter((p) => (p.startsWith('/uploads/') || p.startsWith('http://') || p.startsWith('https://') || p.startsWith('file://') || p.startsWith('content://')) && isValidImage(p));
            const photosFromTable = recordPhotos.filter((p) => isValidImage(p));
            // 合并去重
        ...
        const result = {
            ...
            defects: defects || [],
            photos: photos || [],
            checkedCount: records?.filter((r) => r.result !== 'unchecked').length || 0,
...
router.post('/', async (req, res) => {
    try {
        const { orderNo, productNo, supplier, product, quantity, aql, sampleSize, templateId, checklist_id, supplier_name, product_name, batch_number, inspection_date, inspector, notes } = req.body;
...
router.post('/:id/submit', async (req, res) => {
    try {
        ...
        if (!isSupabaseConfigured()) {
            ...
            const passedItems = records.filter((r) => r.result === 'passed').length;
...
router.patch('/:id/checklist-items/:itemId', async (req, res) => {
    try {
        ...
        if (barcodeCodes !== undefined)
            recordUpdateData.barcode_codes = barcodeCodes;
        if (codes !== undefined)
            recordUpdateData.barcode_codes = codes;
        if (photos !== undefined)
            recordUpdateData.photos = photos;
...
router.get('/:id/export-pdf', async (req, res) => {
    try {
        ...
        const checklistItems = records.map((record) => ({
